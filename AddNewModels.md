...
In the `src/models` directory the classes which implement each model. Create a descendents of the class in `modelApi.ts`.
...
Also add additional information if needed.
...
a new store in `src/stores/ToolSpecific.ts`.

## Create a configuration for your model

Like with the other models you need to create a configuration for your model. This is done in `src/components/Sidebar/ToolMenu`.
Check the other models for examples. You need to add a empty component for a delete button to show up.
...
If you want to persist configurations for your model you need to add new options to `ToolConfig` in `src/graphs/serializedData.ts`.
Also in `getToolConfigData` inside `src/components/Navbar/LoadSaveAction.vue` you need to add the new options to the function to be serialized.
