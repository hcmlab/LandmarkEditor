 ...
 */
export class WebServiceFaceModel extends MediapipeFaceModel {
  ...
  async detect(
    imageFile: ImageFile,
    _deletedFeatures: Set<BodyFeature> | undefined
    ...
    return getFingerprint().then(async (fingerprint) => {
      const request: RequestInfo = new Request(this._url + '/detect?__id__=' + fingerprint, {
        method: 'POST',
        body: formData
      });
      return fetch(request)
